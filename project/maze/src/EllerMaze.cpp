...
void EllerMaze::initializeRowSets ( RowSets & _rowSets, int & _nextSetId ) const
{
...
void EllerMaze::mergeHorizontalWalls ( int _row, RowSets & _rowSets )
{
...
void EllerMaze::createVerticalConnections ( int _row, RowSets & _rowSets )
{
...
void EllerMaze::prepareTheNextRow ( int _row, RowSets & _rowSets, int & _nextSetId )
{
...
void EllerMaze::handleTheLastRow ( RowSets & _rowSets )
{
    for ( FlexibleIterator it( m_size - 1, m_leftToRight ); it.hasNext(); )
    {
        int col = it.next();
...
void EllerMaze::rotateGrid ( RotationDirection direction, int _times )
{
    _times = _times % 4;
    if ( _times == 0 ) return;
